...
def prepare_media_for_sync(media_path):
    """
    Assign the Sync Tag video to VLC without starting playback.
    This is intentionally different from load_and_pause().
    Sync preparation must never call player.play(), because
    the video must remain visually unchanged until PLAY_AT.
    """
    media = instance.media_new(media_path)
    player.set_media(media)
...
def prepare_sync_video(sync_tag, sync_id):
    ...
    if not sync_tag:
        ...
        return
    log(
        f"Preparing Sync Tag '{sync_tag}' (ID: {sync_id})",
        "SYNC"
    )
    try:
        ...
        for video in order:
            ...
            if not video_path.exists():
                log(
                    f"Sync Tag '{sync_tag}' matched '{filename}', "
        ...
        if matching_video is None:
            log(
                f"Sync Tag '{sync_tag}' not available on this Secondary.",
                "SYNC"
            )
            mqtt_client.publish(
        ...
        log(
            f"Updated selected_video for Preview: {matching_video.name}",
            "SYNC"
        )
        ...
        log(
            f"Prepared '{matching_video.name}' for Sync Tag "
        ...
        log(
            f"READY sent for Sync Tag '{sync_tag}' (ID: {sync_id})",
            "SYNC"
        )
    except Exception as e:
        log(
            f"Failed to prepare Sync Tag '{sync_tag}': {e}",
            "ERROR"
        )
        mqtt_client.publish(
            mqtt_client.TOPIC_SYNC_STATUS,
...
def scheduled_play(start_at_ms, sync_id):
    ...
    if current_sync_id != sync_id:
        log(
            f"Ignoring play_at. Sync ID mismatch. "
            f"Prepared: {current_sync_id}, Received: {sync_id}",
            "SYNC"
        )
        return
    if not current_sync_path:
        log(
            f"Ignoring play_at. No prepared video for Sync ID {sync_id}.",
            "SYNC"
        )
        return
    ...
    if not Path(media_path).exists():
        log(
            f"Ignoring play_at. Prepared video no longer exists: {media_path}",
            "SYNC"
        )
        return
    log(
        f"Playing prepared Secondary video: {Path(media_path).name}",
        "SYNC"
    )
    ...
    log(
        f"Waiting approximately {max(0, delay_ms):.3f} ms",
        "SYNC"
    )
    if delay_ms > 10:
    ...
    log(
        f"Secondary target reached. Actual: {actual_ms:.3f} ms "
        f"Difference: {actual_ms - start_at_ms:+.3f} ms",
        "SYNC"
    )
    player.play()
    log(
        f"Secondary VLC PLAY command executed for Sync ID {sync_id}.",
        "SYNC"
    )
...
def on_message(client, userdata, msg):
    ...
    if command == "prepare_sync":
        ...
        try:
            ...
            return
        log(
            f"Received scheduled PLAY for {start_at_ms:.3f} ms "
            f"(Sync ID: {sync_id})",
            "SYNC"
        )
        with scheduled_play_lock:
