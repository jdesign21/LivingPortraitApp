...
def set_sync_status_handler(handler):
    global sync_status_handler
    sync_status_handler = handler
...
def on_message(client, userdata, msg):
    ...
    if msg.topic == TOPIC_SYNC_STATUS:
        if sync_status_handler:
            sync_status_handler(client, userdata, msg)
        return
    ...
    if not isinstance(sync_tags, list):
        ...
        publish_sync_tags_ack(False, "Invalid Sync Tag list received.")
        return
    ...
    try:
        ...
        for video in order:
            ...
            if video_sync_tag and video_sync_tag.lower() not in valid_tags:
                video["sync_tag"] = ""
        ...
        log(f"Received and saved {len(sync_tags)} Sync Tag(s).", "SYNC TAGS")

        if assignments_removed:
            log(f"Cleared {assignments_removed} invalid video assignment(s).", "SYNC TAGS")

        publish_sync_tags_ack(True, f"Processed {len(sync_tags)} Sync Tag(s).")

    except Exception as e:
        log(f"Failed to process Sync Tags: {e}", "ERROR")
        publish_sync_tags_ack(False, str(e))
...
def publish_sync_tags_ack(success, message=""):
    ...
    try:
        ...
        log(f"Unable to determine Secondary IP for Sync Tag acknowledgment: {e}", "ERROR")

    log(f"Sending Sync Tag acknowledgment from {secondary_name} ({secondary_ip})", "SYNC TAGS")
...
def handle_sync_tags_ack(msg):
    ...
    try:
        ...
        if success:
            log(f"{secondary_name} ({secondary_ip}) successfully processed Sync Tags.", "SYNC TAGS")
        else:
            log(f"{secondary_name} ({secondary_ip}) failed to process Sync Tags: {message}", "SYNC TAGS")

    except Exception as e:
        log(f"Failed to process Sync Tag acknowledgment: {e}", "ERROR")
...
if is_enabled():
    if role == "primary":
        ...
        log(f"MQTT connection status: {client.is_connected()}", "MQTT")
...
def publish(topic, data):
    ...
    if not client.is_connected():
        log(f"MQTT not connected; skipping publish to {topic}.", "ERROR")
        return

    info = client.publish(topic, data)
    log(f"MQTT publish result: {info.rc}", "MQTT")
